...
!=============================================================================
subroutine apply_thermalbath(particle,igrid_particle,ipe_particle,BC_applied)

use constants
include 'amrvacdef.f'
type(particle_node), intent(inout)        :: particle
double precision                          :: s1, s2, t1, t2, prob, prob2, theta, theta2, zboundarymax,zboundarymin
integer                                   :: seed
integer, intent(inout)                    :: igrid_particle, ipe_particle
logical,intent(out)                       :: BC_applied
integer                                   :: idim
...
!> if 2.5D then no BC_applied in z-direction. However use z-coordinate to apply
!> thermalisation also in 2.5D
...
{#IFDEF PARTICLES_GCA
	!> make number random if more than one processor is used, by multiplying by its momentum and normalising over the speed of light
	s1 = s1*particle%u(1)/(CONST_c*particle%u(3))*(particle%index/nparticles)
        ...
        zboundarymax = 3.0d0  !< artificial z boundary set in 2.5D, should be set in init_particles.t in code dimensions
        zboundarymin = -3.0d0 !< artificial z boundary set in 2.5D, should be set in init_particles.t in code dimensions


  if (particle%x(3) .lt. zboundarymin) then
        particle%x(3) = particle%x(3) + (zboundarymax - (zboundarymin))
        !> thermalise parallel velocity
        prob  = sqrt(-2.0d0*log(1.0-.999999*s1));
        particle%u(1) = sqrt(2.0d0) * UNIT_VELOCITY *prob !< random v//
        particle%u(3) =  one/sqrt(one-(UNIT_VELOCITY/CONST_c)**2) !< gamma from unit velocity
        particle%u(1) = particle%u(3) * particle%u(1) !< random gamma v//
        !BC_applied = .true. !< this does not apply in 2.5D
      end if
      if (particle%x(3) .ge. zboundarymax) then
        particle%x(3) = particle%x(3) - (zboundarymax - (zboundarymin))
        !thermalise parallel velocity
        prob  = sqrt(-2.0d0*log(1.0-.999999*s1));
        particle%u(1) = sqrt(2.0d0) * UNIT_VELOCITY *prob !< random v//
...
!> make number random if more than one processor is used, by multiplying by its
!normalised momentum

        s1 = s1*max(particle%u(1),particle%u(2),particle%u(3))/(sqrt(particle%u(1)**2+particle%u(2)**2+particle%u(3)**2))
        s2 = s2*max(particle%u(1),particle%u(2),particle%u(3))/(sqrt(particle%u(1)**2+particle%u(2)**2+particle%u(3)**2))
        t1 = t1*max(particle%u(1),particle%u(2),particle%u(3))/(sqrt(particle%u(1)**2+particle%u(2)**2+particle%u(3)**2))
        t2 = t2*max(particle%u(1),particle%u(2),particle%u(3))/(sqrt(particle%u(1)**2+particle%u(2)**2+particle%u(3)**2))
      ...
      if (particle%x(3) .lt. zboundarymin) then
        particle%x(3) = particle%x(3) + (zboundarymax - (zboundarymin))
        !> thermalise parallel velocity
        prob  = sqrt(-2.0d0*log(1.0-.999999*s1)); !< random number for x and y  direction
        prob2  = sqrt(-2.0d0*log(1.0-.999999*s2));!< random number for z direction      
        theta = 2.0d0*dpi*t1;                     !< random pitch angle 
        theta2 = 2.0d0*dpi*t2;                    !< random pitch angle for z direction
        particle%u(1) = UNIT_VELOCITY *prob*dcos(theta)/CONST_c  !< random vx
        particle%u(2) = UNIT_VELOCITY *prob*dsin(theta)/CONST_c  !< random vy
        particle%u(3) = UNIT_VELOCITY *prob2*dcos(theta2)/CONST_c !< random vz
        !BC_applied = .true. !< this does not apply in 2.5D
      end if
      if (particle%x(3) .ge. zboundarymax) then
         particle%x(3) = particle%x(3) - (zboundarymax - (zboundarymin))
        !thermalise parallel velocity
        prob  = sqrt(-2.0d0*log(1.0-.999999*s1)); !< random number for x and ydirection
        prob2  = sqrt(-2.0d0*log(1.0-.999999*s2));!< random number for z direction      
        theta = 2.0d0*dpi*t1;                     !< random pitch angle 
        theta2 = 2.0d0*dpi*t2;                    !< random pitch angle for z direction
        particle%u(1) = UNIT_VELOCITY *prob*dcos(theta)/CONST_c  !< random vx
...
{#IFDEF PARTICLES_GCA
	!> make number random if more than one processor is used, by multiplying by its momentum and normalising over the speed of light
	s1 = s1*particle%u(1)/(CONST_c*particle%u(3))
...
!normalised momentum
    ...
    select case(idim)
      ...
      if (particle%x(^D) .lt. xprobmin^D) then
        ...
        !> thermalise parallel velocity
        prob  = sqrt(-2.0d0*log(1.0-.999999*s1)); !< random number for x and ydirection
        prob2  = sqrt(-2.0d0*log(1.0-.999999*s2));!< random number for zdirection      
        theta = 2.0d0*dpi*t1;                     !< random pitch angle 
        theta2 = 2.0d0*dpi*t2;                    !< random pitch angle for z direction
        particle%u(1) = UNIT_VELOCITY *prob*dcos(theta)/CONST_C  !< random vx
        particle%u(2) = UNIT_VELOCITY *prob*dsin(theta)/CONST_c  !< random vy
...
end subroutine apply_thermalbath
!=============================================================================
